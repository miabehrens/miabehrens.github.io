(function ($, window) {
  window.norgram = {
    ...
    bindMenu: function () {
      ...
      $("div.modal-backdrop").click(toggleMenu);

      $("#menu").on("click", "a", function (event) {
        let activeMenuItem = $menuList.find("li.active"),
          clickedMenuItem = $(event.currentTarget).parent();

        if (clickedMenuItem.is(".sub-menu-item")) {
          $menuList
            .find("li.sub-menu-item.active-sub")
            .removeClass("active-sub");
          clickedMenuItem.addClass("active-sub");
        } else if (!clickedMenuItem.is(activeMenuItem)) {
    ...
    bindCopyColorLinks: function () {
      new ClipboardJS("a.copy-color", {
        text: function (trigger) {
          return trigger.innerText;
        },
      });

      $("div.section-color-table").on(
        "click",
        "a.copy-color",
        function (event) {
          $(event.currentTarget)
        ...
        handleHeader = function () {
          if ($hero.length > 0) {
            requestAnimationFrame(function () {
              var overHero =
                window.scrollY < $hero.offset().top + $hero.height();
